...
public class ParquetSchemaUtil {
  ...
  public static MessageType pruneColumns(MessageType fileSchema, Schema expectedSchema) {
    ...
    // Use getProjectedIdsExcludingStructs to avoid selecting entire structs when only
    // nested fields are projected (e.g., SELECT struct.field should not select entire struct)
    Set<Integer> selectedIds = TypeUtil.getProjectedIdsExcludingStructs(expectedSchema);
    return (MessageType)
